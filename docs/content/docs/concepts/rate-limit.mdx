...
By default, the rate limters uses session token to identify the user. Since, the session token is signed and contains user information, it can be used to identify the user across different requests. 

If the session token is not available, the rate limter will use the IP address to identify the user. This is done by hashing the IP address and using the hashed value to identify the user.

You can use the `getKey` function to customize the key that is used to identify the user. This can be useful if you want to use a different identifier for the user.
...
export const auth = betterAuth({
    rateLimit: {
        getKey: async (context) => {
            // return the key that is used to identify the user
        }
    }
})
```
